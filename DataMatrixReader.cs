...
using ZXing;
using ZXing.Common;
using Corex.VisionLib.DataMatrixReader.Internal;
    ...
    public sealed class DataMatrixReader : Reader
    {
        ...
        public Result decode(BinaryBitmap image, IDictionary<DecodeHintType, object> hints)
        {
            ...
            if (hints != null && hints.ContainsKey(DecodeHintType.PURE_BARCODE))
            {
                ...
                if (bits == null)
                    return new Result("BITS_ERROR", new byte[1], NO_POINTS, BarcodeFormat.DATA_MATRIX, Status.NotFound);
            ...
            else
            {
                ...
                if (detectorResult == null)
                    return new Result("DETECTOR_ERROR", new byte[1], NO_POINTS, BarcodeFormat.DATA_MATRIX, Status.NotFound);
                decoderResult = decoder.decode(detectorResult.Bits);
                points = detectorResult.Points;
            }
            if (decoderResult == null)
                return new Result("DECODE_ERROR", new byte[1], NO_POINTS, BarcodeFormat.DATA_MATRIX, Status.NotFound);

            Result result = new Result(decoderResult.Text, decoderResult.RawBytes, points, BarcodeFormat.DATA_MATRIX, decoderResult.Status);
