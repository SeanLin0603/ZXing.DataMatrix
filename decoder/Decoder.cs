...
namespace Corex.VisionLib.DataMatrixReader.Internal
{
    /// <summary>
    ...
    public sealed class Decoder
    {
        ...
        public DecoderResult decode(BitMatrix bits)
        {
            ...
            if (parser.Version == null)
                return new DecoderResult(new byte[1], "VERSION_ERROR", null, null, Status.FormatError);

            // Read codewords
            byte[] codewords = parser.readCodewords();
            if (codewords == null)
                return new DecoderResult(new byte[1], "CODEWORDS_ERROR", null, null, Status.FormatError);
            ...
            for (int j = 0; j < dataBlocksCount; j++)
            {
                ...
                if (!correctErrors(codewordBytes, numDataCodewords))
                    return new DecoderResult(new byte[1], "CORRECTION_ERROR", null, null, Status.ChecksumError);
